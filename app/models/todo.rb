...
class Todo < ActiveRecord::Base
    ...
    def to_displayable_view_string
        display_status = completed ? "[x]" : "[ ]"
        display_date = due_today? ? nil : due_date
        display_id = id
        "#{display_id} #{display_status} #{todo_text} #{display_date}"
    ...
    def self.to_displayable_view_list
        all.map {|todo| todo.to_displayable_view_string }.join("\n")
    end
    ...
    def self.show_single(id)
        Todo.find(id).to_displayable_string
        #all.where(id:id).to_displayable_string
    end    
